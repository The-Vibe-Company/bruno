...
import { Notifications } from "@/reglages/Notifications";
import { abonner } from "@/api/push";
import { clePublique } from "@/relances/push";

export const dynamic = "force-dynamic";

/** Les Réglages : les Créneaux et où les Relances arrivent, la liste des Affectations, l'apparence, le compte. Rien d'autre — et surtout pas qui est sur quoi. */
export default async function Reglages() {
  const session = await sessionCourante();
  if (!session) redirect("/api/auth/google"); // le layout l'a déjà fait ; TypeScript veut la garantie
  const [creneaux, affectations, projets, appareils, [moi], jar] = await Promise.all([lister(session), listerAffectations(session), listerProjets(session), abonner.mesAppareils(session), db.select().from(membre).where(eq(membre.id, session.membreId)), cookies()]);
  ...
  return (
    <>
      ...
      <main className="grid min-h-0 flex-1 grid-cols-1 content-start gap-16 overflow-y-auto px-10 py-8 md:grid-cols-2">
        <div className="flex flex-col gap-12"><Creneaux initiaux={creneaux} /><Notifications clePublique={clePublique()} initiaux={appareils} /></div>
