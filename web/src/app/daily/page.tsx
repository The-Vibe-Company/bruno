 ...
 */
export default async function Daily({ searchParams }: { searchParams: Promise<{ membre?: string }> }) {
  ...
  const [sante, dujour, delaVeille, finies, feu, membres] = await Promise.all([
    ...
    lister(session, { bucket: "sur_le_feu", inclureTerminees: false, assigneId: courant || undefined }),
  ...
  const deLui = <T extends { membreId: string }>(l: T[]) => (courant ? l.filter((m) => m.membreId === courant) : l);
  const tachesHier: TacheFinie[] = finies
    .filter((t) => !courant || t.assigneId === courant)
    .map((t) => ({ id: t.id, titre: t.titre, etat: t.etatTerminal!, assigne: t.assigneId ? { nom: nomDe.get(t.assigneId) ?? "?" } : null }));
  const tachesFeu: TacheDuJour[] = feu.map((t) => ({
  ...
  return (
    <Coquille initiale={session.nom.charAt(0).toUpperCase()} page="daily">
      <header className="flex h-16 flex-none items-center gap-7 border-b border-bord-2 px-8">
        ...
        <FiltreMembre membres={membres} courant={courant} base="/daily" />
        <div className="flex-1" />
        <Sante aTrier={sante.aTrier} reportees={sante.reportees} seuil={SEUIL_SIGNAL} />
      </header>
      <Affectations membres={deLui(dujour)} moiId="" choix={[]} />
