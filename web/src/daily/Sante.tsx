...
export function Sante({ aTrier, reportees, seuil }: { aTrier: number; reportees: number; seuil: number }) {
  return (
    <div className="flex items-center">
      ...
      <span className="mx-3 h-6 w-px bg-bord-faible" />
...
function Signal({ n, libelle }: { n: number; libelle: string }) {
  return (
    <span className="flex items-baseline gap-2.5">
      <span className={`text-[26px] font-light leading-none tracking-tighter tabular-nums ${n > 0 ? "text-accent" : "text-texte-faible"}`}>{n}</span>
      <span className="text-[13px] text-texte-sourd">{libelle}</span>
    </span>
  );
}
