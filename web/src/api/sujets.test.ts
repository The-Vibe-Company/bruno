...
import { lister, poser, supprimer } from "./sujets";
...
describe("les Sujets du Weekly", () => {
  it("se posent sur la liste d'un autre : c'est une réunion, pas un dossier personnel", async () => {
    const pose = await poser(ctx(), { lundi: LUNDI, membreId: stan, texte: "Le point sur AFP" });
    expect(pose.membreId).toBe(stan);
    expect(pose.auteur).toBe("Antoine");
  });

  it("ne débordent pas d'une semaine sur l'autre", async () => {
    await poser(ctx(), { lundi: "2026-09-21", membreId: antoine, texte: "La semaine suivante" });
  ...
  it("gardent un lien tel quel — c'est comme ça qu'on partage un skill", async () => {
    const pose = await poser(ctx(), { lundi: LUNDI, membreId: antoine, texte: "mon skill https://thecompanion.sh/skills" });
    expect(pose.texte).toContain("https://thecompanion.sh/skills");
  });

  it("se retirent sans cérémonie", async () => {
    const pose = await poser(ctx(), { lundi: LUNDI, membreId: antoine, texte: "À retirer" });
  ...
  it("refusent un Membre d'un autre Espace", async () => {
    await expect(poser(ctx(), { lundi: LUNDI, membreId: randomUUID(), texte: "Ailleurs" })).rejects.toThrow(/Membre/);
  });
